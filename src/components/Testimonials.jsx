...
export default function Testimonials({ testimonialsList, activeIdx, setActiveIdx }) {
  ...
  return (
    <section id="testimonials" className="relative pt-8 pb-4 bg-[#E2D5F3] text-gray-900 overflow-hidden">
            ...
            {testimonialsList.map((t, idx) => (
              ...
              >
                ...
                <div className="border-t border-purple-100 pt-3 mt-auto">
                  <p className="font-serif font-bold text-xs sm:text-sm text-[#3E0844]">{t.name}</p>
            ...
            {[...Array(maxIdx + 1)].map((_, idx) => (
              <button
                ...
                className={`h-3 rounded-full transition-all duration-300 cursor-pointer ${
                  idx === activeIdx
                    ? 'w-7 bg-[#3E0844] border border-[#3E0844]/20'
                    : 'w-3 bg-purple-300 hover:bg-[#6B1736]/40 border border-purple-400/30'
                }`}
        ...
        <div className="bg-white rounded-[1.5rem] sm:rounded-[2rem] p-3.5 sm:p-8 border-2 border-[#3E0844]/40 w-full">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            ...
            <div className="lg:col-span-5 space-y-4">
              <span className="inline-flex items-center gap-1.5 text-xs font-bold tracking-widest text-[#6B1736] uppercase bg-purple-50 px-3.5 py-1.5 rounded-full border border-purple-100">
                <Sparkles className="w-3.5 h-3.5 text-[#6B1736]" /> Join the Circle
              </span>
              ...
              {showSuccess ? (
                ...
                <form onSubmit={handleTestimonialSubmit} className="bg-purple-50/25 border border-purple-100/65 p-3.5 sm:p-6 rounded-xl sm:rounded-2xl space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-white border border-purple-200/80 rounded-xl px-4 py-3 text-xs font-bold text-[#1C0320] placeholder-gray-400 focus:outline-none focus:border-[#3E0844]/60 focus:ring-1 focus:ring-[#3E0844]/20 transition-all shadow-sm"
                      />
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 bg-white border border-purple-200/80 w-fit px-4 py-2.5 rounded-xl shadow-sm">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <button
                          key={star}
                          type="button"
                      ...
                      <span className="text-[10px] font-bold text-[#3E0844] uppercase tracking-wider ml-2">{newRating} Stars</span>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-white border border-purple-200/80 rounded-xl px-4 py-3 text-xs font-semibold text-[#1C0320] placeholder-gray-400 focus:outline-none focus:border-[#3E0844]/60 focus:ring-1 focus:ring-[#3E0844]/20 transition-all shadow-sm leading-relaxed"
                    />
                  </div>
