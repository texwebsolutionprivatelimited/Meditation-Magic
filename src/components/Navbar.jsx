...
export default function Navbar({ onOpenModal, cart = [], onRemoveFromCart, onClearCart, onAddToCart }) {
  ...
  const courseDropdownItems = [
    { name: 'All Programs', href: '/courses', icon: <BookOpen className="w-5 h-5" /> },
    { name: 'Courses', href: '/courses?type=Courses', icon: <GraduationCap className="w-5 h-5" /> },
    { name: 'Workshops', href: '/courses?type=Workshops', icon: <Award className="w-5 h-5" /> },
    { name: '1-to-1 Sessions', href: '/counseling', icon: <HeartHandshake className="w-5 h-5" /> },
  ];
  ...
  // Active check helper for dropdown items
  const isDropdownItemActive = (item) => {
    if (item.name === 'All Programs') {
      return path === '/courses' && !location.search;
    }
    if (item.name === 'Courses') {
      return (location.search.includes('type=Courses')) || (currentCourseType === 'Course');
    }
    ...
    <>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="hidden xl:flex items-center gap-3 lg:gap-5">
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                {/* Dropdown Panel */}
                {isCoursesOpen && (
                  <div className="absolute top-full left-0 mt-2 w-52 bg-white rounded-2xl shadow-xl border border-purple-100 overflow-hidden z-50 animate-fade-in">
                    <div className="p-1.5">
                    ...
                    {/* Bottom accent */}
                    <div className="h-1 bg-gradient-to-r from-[#3E0844] to-[#6B1736]" />
                  </div>
                )}
              </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <a
                ...
                className="hidden xl:flex items-center gap-1.5 border border-white/40 hover:border-white text-white font-semibold text-xs px-4 py-2 rounded-lg transition-all duration-300 hover:bg-white hover:text-[#3E0844] active:scale-95"
              >
              ...
              <button
                ...
                className="xl:hidden text-white/80 hover:text-[#F5D28E] p-2 rounded-lg hover:bg-white/10 transition-all duration-200"
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-50 flex">
          ...
          />
          <div className="relative w-80 max-w-full bg-[#3E0844] shadow-2xl p-6 flex flex-col justify-between border-r border-white/10 z-10 text-white overflow-y-auto">
            <div>
              {/* Drawer Header */}
              <div className="flex items-center justify-between border-b border-white/10 pb-4 mb-6">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="font-serif text-white font-extrabold text-sm uppercase tracking-wide">Meditation Magic</div>
                    <div className="text-[9px] text-white/50 tracking-widest uppercase">With Neelam Arora</div>
                  </div>
                </div>
                <button
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="text-white/60 hover:text-[#F5D28E] transition-colors"
                >
              ...
              <div className="flex flex-col gap-2">
                ...
                {isMobileCoursesOpen && (
                  <div className="ml-4 border-l border-white/10 pl-4 flex flex-col gap-1">
                    {courseDropdownItems.map((item) => {
                      const isActive = isDropdownItemActive(item);
                      return (
                        <a
            ...
            <a
              ...
              rel="noopener noreferrer"
              onClick={() => setIsMobileMenuOpen(false)}
              className="mt-6 border border-white/40 hover:border-white text-white w-full py-3.5 rounded-xl flex items-center justify-center gap-2 text-sm font-bold shadow-lg transition-all hover:bg-white hover:text-[#3E0844] text-center block whitespace-nowrap"
            >
      ...
      {isCartOpen && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-80 max-w-full bg-white shadow-2xl flex flex-col z-10 overflow-y-auto">
            {/* Cart Header */}
            <div className="flex items-center justify-between p-5 border-b border-purple-100 bg-[#3E0844] text-white">
              <div className="flex items-center gap-2">
                <ShoppingCart className="w-5 h-5" />
                <span className="font-serif font-bold text-base">Your Cart</span>
                {cart.length > 0 && (
                  <span className="bg-[#F5D28E] text-[#3E0844] text-[10px] font-black px-2 py-0.5 rounded-full">
            ...
            <div className="flex-1 p-4 space-y-3">
              {cart.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  <ShoppingCart className="w-12 h-12 text-[#3E0844]/20 mb-3" />
                  <p className="font-serif font-bold text-[#3E0844] text-sm">Your cart is empty</p>
                  <p className="text-gray-500 text-xs mt-1">Add energized crystals to begin</p>
                </div>
              ) : (
                cart.map((item) => (
                  <div key={item.id} className="flex gap-3 bg-purple-50/50 border border-purple-100/60 rounded-2xl p-3">
                    <img src={item.image} alt={item.title} className="w-14 h-14 rounded-xl object-cover flex-shrink-0 border border-purple-100" />
                    <div className="flex-1 min-w-0">
                      <p className="font-serif font-bold text-xs text-[#1C0320] leading-snug line-clamp-2">{item.title}</p>
                      <p className="text-[#3E0844] font-black text-sm mt-1">₹{item.price.toLocaleString('en-IN')}</p>
                      <div className="flex items-center gap-2 mt-2">
                        <button onClick={() => onRemoveFromCart(item.id)} className="w-6 h-6 rounded-full bg-[#3E0844] text-white text-xs flex items-center justify-center hover:bg-[#6B1736] transition-colors font-bold">
                          −
                        </button>
            ...
            {cart.length > 0 && (
              <div className="p-4 border-t border-purple-100 space-y-3">
                <div className="flex justify-between text-sm font-bold text-[#1C0320]">
                  <span>Total</span>
                  <span className="text-[#3E0844] font-black">₹{cart.reduce((sum, item) => sum + item.price * item.quantity, 0).toLocaleString('en-IN')}</span>
                </div>
                <a
                  href={`https://wa.me/919829156812?text=Hello%20Neelam%20Arora%20Team!%20I%20would%20like%20to%20order:%20${cart.map(i => `${i.title}%20(x${i.quantity})`).join('%2C%20')}.%20Please%20guide%20me%20on%20payment%20and%20delivery.`}
                  target="_blank"
                  rel="noopener noreferrer"
                ...
                <button
                  onClick={onClearCart}
                  className="w-full border border-purple-200 text-[#3E0844]/70 font-bold py-2.5 rounded-xl text-xs uppercase tracking-wider transition-all hover:bg-purple-50 whitespace-nowrap"
                >
                  Clear Cart
                </button>
              </div>
